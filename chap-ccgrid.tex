...
For PV panels, it is considered a lifetime of 30 years, and manufacturing 1 $m^2$ emits 250 kg of \ch{CO2}-eq, inspired from real measurements~\cite{YUE2014pv_carbon}. To compute the emissions in the form of $g\,\ch{CO2}\text{-}eq/kWh$ as stated in Section~\ref{sec:footprintmodel_ccgrid}, we considered the total solar irradiation that was produced during the year 2021 multiplied by 30 (to account for the PV module lifetime of 30 years). For the electrical grid, we also considered the real-world data of the carbon footprint ($g\,\ch{CO2}\text{-}eq/kWh$). Table~\ref{tab:carbonfootprint} lists the carbon emission values for each region.
