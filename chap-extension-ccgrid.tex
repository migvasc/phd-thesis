...
The carbon footprint model (presented in Section~\ref{sec:footprintmodel_ccgrid} needs to be updated to model the \ch{CO2} emissions from the whole life cycle of the renewable infrastructure, and Table~\ref{tab:notations_lifecycle_model} presents the new notations used in the updated model.  The previous model only considered the \ch{CO2} emissions from the manufacturing phase, and now we use the emissions of the energy produced (or delivered in the case of batteries) in $g\,\ch{CO2}-eq.Wh/kWh$ during the whole life cycle---which includes operation and recycling or discarding. Equation~\eqref{eq:fppv_lca} models the new footprint for the PV panels, and the main difference is the $pvCO2LC$ input that represents the life-cycle emissions ($g\,\ch{CO2}\text{-}eq/kWh$).

\begin{table}[!t]
\caption{New notations for the updated carbon footprint model  for each $DC^d$ ($1\leq d\leq D$) during time slot $k$ ($0\leq k< K$)}
\label{tab:notations_lifecycle_model}
\begin{center}
\begin{tabular}{l p{11cm}}

% Notations & Meaning\\
$pvCO2LC$ & Carbon footprint (in grams of \ch{CO2}-eq) to produce 1 kWh from a photovoltaic panel considering all the phases of its life cycle \\
$batCO2LC$ & Carbon footprint (in grams of \ch{CO2}-eq) to deliver (or discharge) 1 kWh from a lithium-ion battery  considering all the phases of its life cycle \\
$SUNRISE^d$ & A set with all the instants of time when the sun starts shining throughout the year (1 value per day) in the location of data center $d$ considering its time zone. \\
$FPbat_k^d$ & Carbon emissions (in grams of \ch{CO2}-eq) of discharging energy from the battery of DC $d$ during time slot $k$ \\
\end{tabular}
\end{center}
\end{table}
...
\begin{equation} \label{eq:fbat_lca}
   FPbat^d_k =  Pdch^d_k \times batCO2LC \times \Delta t
 \end{equation}
...
\begin{figure}[h]  
  \centering
   {\epsfig{file = images/pv_daily_ghi_avg.pdf, width = \linewidth}}
...
\begin{figure}[h]
  \centering
  {\epsfig{file = images/pv_sizing_after_lca.pdf, width = \textwidth}}
...
\begin{figure}[h]
  \centering
  {\epsfig{file = images/bat_sizing_after_lca.pdf, width = \textwidth}}
...
The electrical model (see~\ref{sec:electrical_model_ccgrid}) needs to be updated to include the new renewable infrastructure, and we show in Table~\ref{tab:notations_wt_model} the new notations used in the updated model. We consider a new variable $WT^d$ representing how many wind turbines will be built at each DC location. The number of wind turbines (WT) is an integer number (we cannot produce 1.2 WT for example). However, to avoid increasing the computational complexity of the LP by including integer variables, we made a relaxation in our modeling to allow the WT variable to be linear. 

\begin{table}[t]
\caption{New notations for the updated electrical and carbon footprint models  for each $DC^d$ ($1\leq d\leq D$) during time slot $k$ ($0\leq k< K$)}
\label{tab:notations_wt_model}
\begin{center}
\begin{tabular}{l p{11cm}}

% Notations & Meaning\\
$WT^d$ & Number of Wind Turbines at DC $d$ \\
$Pwt^d_k$ & Power generated from the wind turbines at DC $d$ during time slot $k$ \\
$V^d_k$ & Wind speed (m/s) at the location of DC $d$ during time slot $k$ \\
$Vici$ & Cut in wind speed (m/s), in which the WT starts producing power \\
$Vco$ & Cut out wind speed (m/s), in which the WT does not generate more power \\
$Pr$ & Rated power of the WT (in Watts) \\
$wtCO2LC^d$ & Carbon emissions (in grams of \ch{CO2}-eq) of producing 1 kWh of energy from wind turbines taking into account all their life cycle \\
$FPwt_k^d$ & Carbon emissions (in grams of \ch{CO2}-eq) of producing power from the wind turbines of DC $d$ during time slot $k$ \\
\end{tabular}
\end{center}
\end{table}
...
\begin{figure}[h]
  \centering
  {\epsfig{file = images/pv_sizing_after_wt.pdf, width = \textwidth}}
...
\begin{figure}[h]
  \centering
  {\epsfig{file = images/bat_sizing_after_wt.pdf, width = \textwidth}}
...
\begin{figure}[H]
  \centering
  {\epsfig{file = images/pv_intermitency.pdf, width = \textwidth}}
...
\begin{figure}[H]
  \centering
  {\epsfig{file = images/wt_intermitency.pdf, width = \textwidth}}
...
Table~\ref{tab:co2_10y} presents the results in terms of \ch{CO2} emissions for the renewable infrastructure that included both PV panels and wind turbines, and in Table~\ref{tab:co2_10y_pv_only} only PV panels are considered.
 ...
 \begin{table}[H]   
  \caption{Emissions (in kilotons of \ch{CO2}-eq) for the different scenarios---renewable infra only have PV panels and batteries.} \centering
 ...
 \begin{table}[H]   
  \caption{Emissions (in kilotons \ch{CO2}-eq) for the different scenarios---renewable infra only have wind turbines and batteries.} \centering
...
\begin{table}[H]
  \caption{Emissions (in kilotons of \ch{CO2}-eq) for the different scenarios---renewable infra has PV panels, wind turbines and batteries.} \centering
    \label{tab:co2_10y}
      \begin{tabular}{|l|r|r|}        
        \hline        
        \textbf{Sizing Scenario} &  \textbf{Emissions } & \textbf{Diff. to baseline (\%) } \\
...
\begin{table}[t]
\caption{New notations for the updated workload model  for each $DC^d$ ($1\leq d\leq D$) during time slot $k$ ($0\leq k< K$)}
\label{tab:notations_wt_model}
\begin{center}
\begin{tabular}{l p{11cm}}

% Notations & Meaning\\
$\alpha$ & Ratio of the total workload that can be delayed (in percentage) \\
$\beta$ & Number of time slots in which the execution of the workload can be delayed (the workload can be delayed up to $\beta$ time slots) \\
$allocd_k$ & Number of CPU cores required to execute the workload that was not delayed at the location of DC $d$ during time slot $k$ \\
$delay_{k,i}^d$ & Number of CPU cores required to execute the workload that was delayed at the location of DC $d$ from the time slot $k$ ($1 \leq i \leq \beta)$   \\
\end{tabular}
\end{center}
\end{table}
...
\begin{table}[t]
\caption{Main notations for the monetary costs model  for each $DC^d$ ($1\leq d\leq D$) during time slot $k$ ($0\leq k< K$)}
\label{tab:notations_wt_model}
\begin{center}
\begin{tabular}{l p{11cm}}

% Notations & Meaning\\
$PriceGrid^d_k$ & Total price of the electricity consumed from the grid of DC $d$ during time slot $k$  \\
$costGrid^d_k$ & Price of buying electricity (in dollars per kWh) from the local electricity grid of DC $d$ during time slot $k$ \\
$PricePV^d_k$ & Total price of the electricity consumed from the solar panels of DC $d$ during time slot $k$ \\
$PVLCOE^d$ &Price of producing electricity (in dollars per kWh) from the solar panels of DC $d$ considering their Levelized Cost of Energy (LCOE) \\
$PriceWT^d_k$ & Total price of the electricity consumed from the wind turbines of DC $d$ during time slot $k$ \\
$WTLCOE^d$ & Price of producing electricity (in dollars per kWh) from the wind turbines of DC $d$ considering their Levelized Cost of Energy (LCOE) \\
$PriceBat^d_k$ & Total price of the electricity discharged from the batteries of DC $d$ during time slot $k$ \\
$BatLCOS^d$ & Price of discharging energy (in dollars per kWh) from the batteries of DC $d$ considering their Levelized Cost of Storage (LCOS) \\
\end{tabular}
\end{center}
\end{table}
...
\begin{figure}[H]
  \centering
  {\epsfig{file = images/pv_sizing_co2_costs.pdf, width = \textwidth}}
...
\begin{figure}[H]
  \centering
  {\epsfig{file = images/bat_sizing_co2_costs.pdf, width = \textwidth}}
...
\begin{figure}[H]
  \centering
  {\epsfig{file = images/wt_sizing_co2_costs.pdf, width = \textwidth}}
...
\begin{table}[h]
\caption{New notations for the IT, electrical and footprint models  for each $DC^d$ ($1\leq d\leq D$) during time slot $k$ ($0\leq k< K$)}
\label{tab:notations_add_replace_servers_model}
\begin{center}
\begin{tabular}{l p{11cm}}

% Notations & Meaning\\
$NS^d$ & Number of new servers to be manufactured at DC $d$ \\
$REP_{gen}^d$ & Number of servers from generation $gen$ to be replaced at DC $d$  \\
$ES_{gen}^d$ & Number of servers from generation $gen$ that are in operation at DC $d$  \\
$cpucores_{gen}$ & Number of CPU cores of the server from generation $gen$ \\
$w_{k,gen}^d$ & Number of CPU cores allocated to the generation $gen$ servers to execute the workload of DC $d$ during the time slot $k$ \\
$\gamma$ & Represents the ratio of the tasks that are service ($0 \leq \gamma \leq 1$) (and remaining $1 - \gamma$ that are batch type) \\
$FPns^d$ & Carbon footprint (in kilograms of \ch{CO2}-eq) of manufacturing new servers at DC $d$ \\
$FPrep^d$ &  Carbon footprint (in kilograms of \ch{CO2}-eq) of replacing servers at DC $d$ \\
$FPes^d$ &  Carbon footprint (in kilograms of \ch{CO2}-eq) of the existing servers in operation at DC $d$ \\
$\epsilon^d$ & Ratio of \ch{CO2}-eq emitted by year considering the server lifetime  ($0 \leq \epsilon \leq 1$) \\
$serversCO2$ & Carbon footprint (in kilograms of \ch{CO2}-eq) of manufacturing one new server \\
$age_{gen}$ & Age of the generation $gen$ servers in years \\
$\mu$ & Ratio of the workload that will be executed in the servers of old generations ($0 \leq \mu \leq 1$) \\
$lowboundbat^d$ & Capacity of the batteries (in kWh) of DC $d$ that is fixed during the sizing process \\
$lowboundpv^d$ & Area of the solar panels (in m²) of DC $d$ that is fixed during the sizing process \\

$lowboundwt^d$ & Number of wind turbines of DC $d$ that is fixed during the sizing process \\
 \end{tabular}
\end{center}
\end{table}
...
The workload model (see~\ref{sec:workload_model_ccgrid}) also needs to be updated. Equation~\eqref{eq:wkgen} models the constraint that all the workload must be executed, and that it can be run on any generation of the servers.
...
In order to evaluate the \ch{CO2} reduction by adding and replacing servers, we need a baseline for comparison. We assume the baseline to be the cloud infrastructure with the servers from the current generations, and new servers are only added if the cloud cannot compute the workload---as it grows year by year, and no server is replaced.
...
\begin{equation}\label{eq:use_old_servers}     
...
The first scenario of the experiments evaluates four years of cloud operation (2017 to 2020) and only considers two generations of servers: servers with the Intel Xeon E5-2660 v2 CPU, which represents the old generation, and servers with the Intel Xeon Platinum 8180 CPU, which represents the new generation. Additionally, the value of $\mu$---that defines a lower bound for the usage of servers from old generations---is equal to $0.99$ in the first scenario. The second and third scenarios of the experiments evaluate five years of cloud operation (2017 to 2021) and consider all the generations of servers listed in Table~\ref{tab:servers_specs}.
...
For the servers of the first generation (year 2013), it is considered that they have been used more than their expected lifetime, and all their \ch{CO2} footprint has been paid. Finally, according to ~\citet{datacenter_as_computer}, the typical lifespan of servers is three to four years. Therefore, we consider the server's expected lifetime to be four years.
...
\paragraph{Scenario 1: evaluating the \ch{CO2} reductions by adopting servers from a new generation } 
...
We observe that the number of servers replaced was low in the scenario where it was possible to add and replace servers (Figure~\ref{fig:dc_comparison_add_replace}), and this can be justified by the fact that manufacturing a new server emits \ch{CO2} and the old servers only emit carbon from the operation, as they are being used over their lifetime. This scenario also manufactured more servers than the baseline: 8.8\% more new servers.
...
\begin{tabular}{|l|r|}
  ...
  \textbf{Scenarios} & \textbf{Emissions ($\mathbf{t\,\ch{CO2}\text{-}eq}$)}   \\
  \hline  
   Optimal solution  &   213 730.59  \\  
  \hline
   Greedy approach   &  246 867.03  \\
  \hline
\end{tabular}
\end{table}
...
Finally, including the life cycle aspect makes our modeling more realistic, and the emissions are still better than the scenario where only power from the local electricity grid is used in terms of carbon footprint---81\% in the scenario with PV and batteries.  
...
The second interesting observation is that by delaying a small portion of the workload (for example, 10\% to 20\%), the reduction is already close to the maximum possible reduction value. Finally, this approach can be used to reduce the impact of renewable sources' intermittency in the sizing process---as we saw differences from 1\% to 12\% in comparison to the optimal scenario on Section~\ref{sec:sensitivity}---and in the context of cloud federations that emit from tens to hundreds of kilotons of \ch{CO2}-eq, a reduction of 4\% is not negligible.
...
% Falar que o modelo permite comparar os ganhos em reducao de co2 para 
We demonstrated through our experiments that our model can be applied to assess the carbon reduction obtained in the cloud federation's operation by adopting servers from a new generation. One can expect the servers from new generations to be more power-efficient and computationally powerful, but we must pay attention to the environmental impact of manufacturing the servers. Furthermore, with the increase of low-carbon-intensive power supplies in the DCs, most of the environmental impact is shifting to Scope 3 of the GHG protocol---which includes manufacturing the IT infrastructure~\cite{gupta2021_chasingcarbon}.
